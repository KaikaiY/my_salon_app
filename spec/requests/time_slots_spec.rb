...
RSpec.describe 'TimeSlots', type: :request do
  ...
  describe 'POST /create' do
    ...
    it '管理者は20分ごとに時間枠をまとめて作成できること' do
      ...
      expect(response).to redirect_to(treatment_day_path(treatment_day))
      expect(treatment_day.time_slots.order(:start_time).map do |slot|
        slot.start_time.strftime('%H:%M')
      end).to eq(%w[10:00 10:20 10:40])
      expect(treatment_day.time_slots.order(:start_time).map do |slot|
        slot.end_time.strftime('%H:%M')
      end).to eq(%w[10:20 10:40 11:00])
    end
    ...
    it '会社責任者は20分ごとに時間枠をまとめて作成できること' do
      ...
      expect(treatment_day.time_slots.order(:start_time).map do |slot|
        slot.start_time.strftime('%H:%M')
      end).to eq(%w[13:00 13:20 13:40])
      expect(treatment_day.time_slots.order(:start_time).map do |slot|
        slot.end_time.strftime('%H:%M')
      end).to eq(%w[13:20 13:40 14:00])
    end
